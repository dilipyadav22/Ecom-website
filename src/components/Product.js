    ...
    <div>
      ...
      mb-4 relative overflow-hidden group transition'>
        ...
        items-center'>
          ...
          items-center'>
            <img className='max-h-[180px]  group-hover:scale-125
            transition duration-700 ' src={image} alt='' />
          </div>
        </div>
        <div className='absolute  top-6 -right-11
        group-hover:right-2 py-1
        flex flex-col items-center justify-center gap-y-2 opacity-0
        group-hover:opacity-100 transition-all duration-300 '> 
          <button onClick={()=> addToCart(data,id)}>
              <div className='flex justify-center items-center
              text-white w-12 h-10 bg-red-500' >
                <BsPlus className='text-3xl' />
              </div>
          </button>
          <Link to={`/product/${id}`} className='w-12 h-10 bg-white flex justify-center
